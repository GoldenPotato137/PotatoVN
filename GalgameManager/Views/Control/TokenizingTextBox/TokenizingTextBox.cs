...
public partial class TokenizingTextBox : ListViewBase
{
    ...
    /// </summary>
    internal static bool IsShiftPressed => InputKeyboardSource.
    ...
    /// </summary>
    internal bool IsControlPressed => InputKeyboardSource.
    ...
    public TokenizingTextBox()
    {
        ...
        _innerItemsSource.InsertIntersperse(_innerItemsSource.Count, _currentTextEdit);
    ...
    private void ItemsSource_PropertyChanged(DependencyObject sender, DependencyProperty dp)
    {
        ...
        if (ItemsSource != null && ItemsSource.GetType() != typeof(InterspersedObservableCollection))
        {
            ...
            if (ReadLocalValue(MaximumTokensProperty) != DependencyProperty.UnsetValue && 
                _innerItemsSource.ItemsSource.Count >= MaximumTokens)
            {
    ...
    private void TokenizingTextBox_PreviewKeyUp(VirtualKey key)
    {
        switch (key)
        {
    ...
    private async Task<bool> TokenizingTextBox_PreviewKeyDown(VirtualKey key)
    {
    ...
    protected override void OnApplyTemplate()
    {
        ...
        {
            Text = "WCT_TokenizingTextBox_MenuFlyout_SelectAll".
                GetLocalized("CommunityToolkit.WinUI.UI.Controls.Input/Resources")
        };
        selectAllMenuItem.Click += (_, _) => SelectAllTokensAndText();
    ...
    {
        TokenizingTextBoxItem? container = ContainerFromItem(_currentTextEdit) as TokenizingTextBoxItem;
        ...
        if (container != null && !(GetFocusedElement().Equals(container._autoSuggestTextBox) || char.IsControl(args.Character)))
        {
            if (SelectedItems.Count > 0)
            {
                ...
                _ = DispatcherQueue.EnqueueAsync(
                    () =>
                    {
                        ...
                        {
                            TokenizingTextBoxItem? lastContainer = ContainerFromItem(_lastTextEdit) as TokenizingTextBoxItem;
                        ...
                        else
                        {
                            ...
                            _innerItemsSource.InsertIntersperse(index, _currentTextEdit);

                            // Need to wait for containerization
                            _ = DispatcherQueue.EnqueueAsync(
                                () =>
                                {
                                    TokenizingTextBoxItem? newContainer = ContainerFromIndex(index) as TokenizingTextBoxItem; // Should be our last text box
            ...
            else
            {
                ...
                if (_innerItemsSource[^1] is ITokenStringContainer textToken)
                {
                    TokenizingTextBoxItem? last = ContainerFromIndex(Items.Count - 1) as TokenizingTextBoxItem; // Should be our last text box
    ...
    protected override void PrepareContainerForItemOverride(DependencyObject element, object item)
    {
        ...
        TokenizingTextBoxItem? tokenItem = element as TokenizingTextBoxItem;
        ...
        removeMenuItem.Click += (_, _) => TokenizingTextBoxItem_ClearClicked(tokenItem, null);
    ...
    /// <summary>
    /// Adds the specified data item as a new token to the collection, will raise the <see cref="TokenizingTextBox.TokenItemAdding"/> event asynchronously still for confirmation.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    /// Clears the whole collection, will raise the <see cref="TokenizingTextBox.TokenItemRemoving"/> event asynchronously for each item.
    /// </summary>
    ...
    public async Task ClearAsync()
    {
        ...
        {
            TokenizingTextBoxItem? container = ContainerFromItem(_innerItemsSource[0]) as TokenizingTextBoxItem;
    ...
    {
        if (ReadLocalValue(MaximumTokensProperty) != DependencyProperty.UnsetValue 
            && (MaximumTokens <= 0 || MaximumTokens <= _innerItemsSource.ItemsSource.Count))
        {
            // No tokens for you
            return;
        }
        ...
        {
            TokenItemAddingEventArgs tokenItemAddingEventArgs = new(str);
            await TokenItemAdding.InvokeAsync(this, tokenItemAddingEventArgs);

            if (tokenItemAddingEventArgs.Cancel)
            {
                return;
            }

            if (tokenItemAddingEventArgs.Item != null)
            {
                data = tokenItemAddingEventArgs.Item; // Transformed by event implementor
            }
        }
        ...
        TokenizingTextBoxItem? last = ContainerFromItem(_lastTextEdit) as TokenizingTextBoxItem;
        last?._autoSuggestTextBox?.Focus(FocusState.Keyboard);
    ...
    /// </summary>
    /// <param name="edit"><see cref="CommunityToolkit.WinUI.UI.Controls.ITokenStringContainer"/> element which is now the main edited text.</param>
    ...
    private void GuardAgainstPlaceholderTextLayoutIssue()
    {
        ...
        FrameworkElement? placeholder = ContainerFromItem(_lastTextEdit)?.FindDescendant("PlaceholderTextContentPresenter");
