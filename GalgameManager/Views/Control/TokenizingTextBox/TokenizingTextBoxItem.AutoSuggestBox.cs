...
/// <summary>
/// A control that manages as the item logic for the <see cref="GalgameManager.Views.Control.TokenizingTextBox.TokenizingTextBox"/> control.
/// </summary>
...
public partial class TokenizingTextBoxItem
{
    ...
    /// Called from <see cref="TokenizingTextBoxItem.OnApplyTemplate"/>
    private void OnApplyTemplateAutoSuggestBox(AutoSuggestBox auto)
    {
        ...
        if (_autoSuggestBox != null)
        {
            ...
            if (Content is ITokenStringContainer { IsLast: true })
            {
                ...
                {
                    Source = Owner,
                    Path = new PropertyPath(nameof(TokenizingTextBox.QueryIcon)),
    ...
    private void AutoSuggestBox_TextChanged(AutoSuggestBox sender, AutoSuggestBoxTextChangedEventArgs args)
    {
        ...
        {
            var lastDelimited = t[^1] == Owner.TokenDelimiter[0];

            var tokens = t.Split(Owner.TokenDelimiter);
            var numberToProcess = lastDelimited ? tokens.Length : tokens.Length - 1;
    ...
    private void AutoSuggestBox_GotFocus(object sender, RoutedEventArgs e)
    {
        ...
        VisualStateManager.GoToState(Owner, TokenizingTextBox.PART_FocusedState, useTransitions: true);
    }
    #endregion
    ...
    {
        TextBlock? maxTokensCounter = (TextBlock?)_autoSuggestBox?.FindDescendant(PART_TokensCounter);
        ...
        {
            InterspersedObservableCollection? itemsSource = ttb.ItemsSource as InterspersedObservableCollection;
        ...
        if (
            Content is ITokenStringContainer { IsLast: true } 
            && ttbi.Owner.ReadLocalValue(TokenizingTextBox.MaximumTokensProperty) != DependencyProperty.UnsetValue)
        {
            ttbi.Owner.TokenItemAdded += OnTokenCountChanged;
            ttbi.Owner.TokenItemRemoved += OnTokenCountChanged;
