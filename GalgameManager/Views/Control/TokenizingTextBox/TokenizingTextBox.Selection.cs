...
/// <summary>
/// Methods related to Selection of items in the <see cref="GalgameManager.Views.Control.TokenizingTextBox.TokenizingTextBox"/>.
/// </summary>
public partial class TokenizingTextBox
{
    ...
    {
        var retVal = false;
        TokenizingTextBoxItem? currentContainerItem = GetCurrentContainerItem();
        ...
        if (currentContainerItem != null)
        {
            ...
            if (index != previousIndex)
            {
                ...
                if (IsShiftPressed)
                {
                    ...
                    var newDistance = Math.Abs(SelectedIndex - index);
                    var oldDistance = Math.Abs(SelectedIndex - previousIndex);
            ...
            () =>
            {
                ...
                foreach (var item in Items)
                {
                    if (item is ITokenStringContainer)
                    {
                        ...
                        TokenizingTextBoxItem? preToken = ContainerFromItem(item) as TokenizingTextBoxItem;
    ...
    private void ClearAllTextSelections(TokenizingTextBoxItem? ignoreItem)
    {
        ...
        foreach (var item in Items)
        {
            if (item is ITokenStringContainer)
            {
                TokenizingTextBoxItem? container = ContainerFromItem(item) as TokenizingTextBoxItem;
    ...
    private bool SelectNewItem(TokenizingTextBoxItem item, int increment, Func<int, bool> testFunc)
    {
        ...
        {
            ListViewItem? newItem = ContainerFromItem(Items[currentIndex + increment]) as ListViewItem;
            newItem!.Focus(FocusState.Keyboard);
    ...
    internal async Task RemoveAllSelectedTokens()
    {
        ...
        {
            TokenizingTextBoxItem? container = ContainerFromItem(SelectedItems[0]) as TokenizingTextBoxItem;
            ...
            if (IndexFromContainer(container) != Items.Count - 1)
            {
                ...
                {
                    TextBox asb = container!._autoSuggestTextBox!;
