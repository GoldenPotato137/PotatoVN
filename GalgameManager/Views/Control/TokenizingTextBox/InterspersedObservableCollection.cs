...
public class InterspersedObservableCollection : IList, IEnumerable<object?>, INotifyCollectionChanged
{
    ...
    private readonly Dictionary<int, object> _interspersedObjects = new();
    ...
    public InterspersedObservableCollection(object itemsSource)
    {
        ...
        {
            WeakEventListener<InterspersedObservableCollection, object, NotifyCollectionChangedEventArgs> 
                weakPropertyChangedListener = new(this)
                {
        ...
        static void ThrowArgumentException() => 
            throw new ArgumentException(
                "The input items source must be assignable to the System.Collections.IList type.");
    }

    private void ItemsSource_CollectionChanged(object _, NotifyCollectionChangedEventArgs eventArgs)
    {
        switch (eventArgs.Action)
        {
            case NotifyCollectionChangedAction.Add:
                // Shift any existing interspersed items after the inserted item
                var count = eventArgs.NewItems?.Count ?? 0;
            ...
            case NotifyCollectionChangedAction.Remove:
                count = eventArgs.OldItems?.Count ?? 0;
    ...
    private int ToInnerIndex(int outerIndex)
    {
        ...
        return outerIndex - _interspersedObjects.Keys.Count(key => key <= outerIndex);
    ...
    private int ToOuterIndex(int innerIndex)
    {
        ...
        foreach (KeyValuePair<int, object> key in _interspersedObjects.OrderBy(v => v.Key))
        {
    ...
    public void InsertIntersperse(int index, object obj)
    {
    ...
    public void Insert(int outerIndex, object obj)
    {
        // Find out our closest index based on interspersed keys
        var index = outerIndex - _interspersedObjects.Keys.Count(key => key < outerIndex); 
    ...
    {
        var i = 0; // Index of our current 'virtual' position
        // var count = 0;
        var realized = 0;
        ...
        if (realized < _interspersedObjects.Count)
        {
            ...
            foreach (KeyValuePair<int, object> keyValue in 
                     _interspersedObjects.Where(kvp => kvp.Key >= i)
                         .OrderBy(kvp => kvp.Key))
            {
    ...
    /// the default value will be KeyValuePair{Key=0, Value=null}
    /// </summary>
    /// <param name="value">Search value</param>
    /// <returns>KeyValuePair or default KeyValuePair</returns>
    private KeyValuePair<int, object?> ItemKeySearch(object? value)
    {
        return _interspersedObjects.FirstOrDefault(kvp => kvp.Value.Equals(value))!;
    }

    public int IndexOf(object value)
    {

        if (ItemKeySearch(value) is { Value: not null } itemKeySearch)
        { 
            return itemKeySearch.Key;
        }
...

    {
        if (ItemKeySearch(value) is { Value: not null } itemKeySearch)
        {
            _interspersedObjects.Remove(itemKeySearch.Key);

            MoveKeysBackward(itemKeySearch.Key, 1); // Move other interspersed items back

            CollectionChanged?.Invoke(this, 
                new NotifyCollectionChangedEventArgs(
                    NotifyCollectionChangedAction.Remove, itemKeySearch.Value, itemKeySearch.Key));
        }
        else
        {
